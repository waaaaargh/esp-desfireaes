...
#ifdef DEBUG
static void dump(const char *prefix, unsigned int len, unsigned char *data)
{
#ifdef ESP_PLATFORM
...
#else
static void fill_random(unsigned char *buf, size_t size)
...
static const char *aes_decrypt(int keylen, const unsigned char *key, unsigned char *iv, unsigned char *out, const unsigned char *in, int len)
{
   if (len <= 0)
      return NULL;
   len = (len + 15) / 16 * 16;
...
#else
static const char *decrypt(EVP_CIPHER_CTX * ctx, const EVP_CIPHER * cipher, int keylen, const unsigned char *key, unsigned char *iv, unsigned char *out, const unsigned char *in, int len)
{
...
static void cmac(df_t * d, unsigned int len, unsigned char *data)
{                               // Process CMAC
#ifdef DEBUG_CMAC
...
const char *df_init(df_t * d, void *obj, df_dx_func_t * dx)
{                               // Initialise
   memset(d, 0, sizeof(*d));
   d->obj = obj;
   d->dx = dx;
...
const char *df_get_file_ids(df_t * d, unsigned long long *ids)
{
   unsigned int rlen;
